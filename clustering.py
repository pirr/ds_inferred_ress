...
pi_df = pd.read_csv('D://Smaga//bitbucket//obj_creator//dict//pi2.csv', delimiter=';', encoding='cp1251')
...
if __name__ == '__main__':
    ...
    X = data[[u'Название объекта', u'Координата центра Y', u'Координата центра X', u'ПИ (перечень для объекта)', u'№ объекта']]
    ...
    names_ratio = np.array(map(names_comp, product(X['name'], repeat=2))).reshape([len(X), len(X)])
    names_ratio_tok = np.array(map(names_comp, product(names, repeat=2))).reshape([len(X), len(X)])
